...
*	file system error
*/
ferror::fsemsgs_type ferror::error_message() const
{
    return util::win::format_error(_code);
}

ferror::fsemsgs_type ferror::print() const
{
...
void ffile::close()
{
    if (vaild())
    {
        ::CloseHandle(reinterpret_cast<void*>(native_id()));
    }
...
bool file_exist(const fpath& name, ferror& ferr)
{
    ferr.clear();
...
void file_remove(const fpath& name, ferror& ferr)
{
    ferr.clear();
...
void file_copy(const fpath& from, const fpath& to, ferror& ferr)
{
    ferr.clear();
...
void file_move(const fpath& from, const fpath& to, ferror& ferr)
{
    ferr.clear();
    if (::MoveFileW(from.c_str(), to.c_str()) == 0)
    {
        ferr = ferror(::GetLastError(), "Can't move file");
    }
}
...
fsize file_size(const fpath& name, ferror& ferr)
{
    ferr.clear();
...
ftime file_time(const fpath& name, ferror& ferr)
{
    ferr.clear();
    ffile file = reinterpret_cast<ffile::native_type>(::CreateFileW(
        name.c_str(),               // lpFileName
...
ffile file_open(const fpath& name, int flags, ferror& ferr)
{
    ferr.clear();
...
void file_read(const ffile& file, void* data_out, int size, ferror& ferr)
{
    ferr.clear();
    if (!file.vaild())
    {
...
void file_write(ffile& file, const void *data, int size, ferror& ferr)
{
    ferr.clear();
    if (!file.vaild())
    {
...
bool file_lock(ffile& file, fsize offset, fsize length, ferror& ferr)
{
    ferr.clear();
...
bool file_unlock(ffile& file, fsize offset, fsize length, ferror& ferr)
{
    ferr.clear();
...
void file_seek(ffile& file, fsize pos, int flag/* = SEEK_SET*/)
{
    ferror ferr;
    file_seek(file, pos, flag, ferr);

    if (ferr)
        throw ferr;
}

void file_seek(ffile& file, fsize pos, int flag, ferror& ferr)
{
    ferr.clear();
    if (!file.vaild())
    {
    ...
    if (::SetFilePointerEx(reinterpret_cast<void*>(file.native_id()), distance, NULL, flag) == 0)
    {
...
fsize file_tell(const ffile& file, ferror& ferr)
{
    ferr.clear();
    if (!file.vaild())
    {
...
fsize file_size(const ffile& file, ferror& ferr)
{
    ferr.clear();
    if (!file.vaild())
    {
...
ftime file_time(const ffile& file, ferror& ferr)
{
    ferr.clear();
    ftime ft = {0, 0, 0};
    ...
    */
    ft.create_time = (ft.create_time - 0x019DB1DED53E8000) / 10;
    ft.access_time = (ft.access_time - 0x019DB1DED53E8000) / 10;
    ft.modify_time = (ft.modify_time - 0x019DB1DED53E8000) / 10;
...
bool file_is_writable(const fpath& name, ferror& ferr)
{
    ferr.clear();
    if (!file_exist(name, ferr))
        return file_is_writable(name, ferr);
...
bool file_is_occupied(const fpath& name, ferror& ferr)
{
    ferr.clear();
    if (!file_exist(name, ferr))
    {
...
bool file_open_with_shell(const fpath& name, ferror& ferr)
...
void directories_remove(const fpath& path, ferror& ferr)
{
    ferr.clear();
...
void directories_create(const fpath& path, ferror& ferr)
{
    ferr.clear();
...
inline void _named_object_authorization(
    const std::wstring& name, ferror& ferr)
{
    ferr.clear();
...
void directory_authorization(const fpath& path, ferror& ferr)
{
