...
TEST(tstring, char_constructor)
{
    ...
    {
        tstring string(u8"我们的征程是星辰大海~~~", format_utf8);
        EXPECT_EQ(u8"我们的征程是星辰大海~~~", string.utf8());
        EXPECT_EQ(_2str("我们的征程是星辰大海~~~", format_utf8), string.string());
        EXPECT_EQ(L"我们的征程是星辰大海~~~", string.wstring());
    }
...
TEST(tstring, wchar_t_constructor)
{
    ...
    {
        tstring string(std::string(u8"我们的征程是星辰大海~~~"), format_utf8);
        EXPECT_EQ(u8"我们的征程是星辰大海~~~", string.utf8());
        EXPECT_EQ(_2str("我们的征程是星辰大海~~~", format_utf8), string.string());
        EXPECT_EQ(L"我们的征程是星辰大海~~~", string.wstring());
    }
...
TEST(tstring, copy_constructor)
{
    ...
    {
        tstring s1(u8"我们的征程是星辰大海~~~", format_utf8);
        tstring string = s1;
        EXPECT_EQ(L"我们的征程是星辰大海~~~", string.wstring());
    }
    ...
    {
        tstring string, s1(u8"我们的征程是星辰大海~~~", format_utf8);
        string = s1;
        EXPECT_EQ(L"我们的征程是星辰大海~~~", string.wstring());
    }
...
TEST(tstring, operator_constructor)
{
    using namespace conv::easy;
    {
        tstring string(u8"Hello {1}!", format_utf8);
        string % "World";
        EXPECT_EQ(L"Hello World!", string.wstring());
    }
    ...
    {
        tstring string(u8"Hello {1} {2}!", format_utf8);
    ...
    {
        tstring string, s1(u8"个体和{1} 高于{2}和{1}!", format_utf8);
        string = s1;
        string % L"互动" % L"流程" % L"工具";
        EXPECT_EQ(L"个体和互动 高于流程和互动!", string.wstring());
    }
